...
### Known gaps
- **ForexFactory never supplies `actual`.** Verified 2026-08-08: 0 of 99 events in `ff_calendar_thisweek.json` carry an actual value, so every macro event is `status: "scheduled"` and the model never sees an actual-vs-forecast print. The brief can say "CPI is due at 13:30", never "CPI came in at 3.4% vs 3.1% expected" — which was the whole reason the macro source was chosen. FRED covers monthly series, not same-day release results. Needs a source that publishes results (FMP/Finnhub economic calendars are both gated on the free tier). The eval's macro assertion is written to activate automatically once released prints appear.
